...
+paddingTop+paddingBottom;});var helpURL=$(content).find('extra').attr('url');if(helpURL){$('.helpText').css('display','block');if(helpURL.split(" ").length>0){if(helpURL.indexOf('/')===0){helpURL=helpURL.substring(1);}
...
maxHeight=document.getElementById(id).offsetHeight;}});var helpURL=$(content).find('extra').attr('url');if(helpURL){$('.helpText').css('display','block');if(helpURL.split(" ").length>0){if(typeof(helpURL)!='undefined'&&helpURL.indexOf('/')==0){helpURL=helpURL.substring(1);}
else{helpURL=Application.getDadName()+"/twbkfrmt.P_DispHelp?pagename_in="+UIDGenerator.getURLFromUID(pageReferrerId);}
Context.helpURL=Application.getProtocol()+"//"+Application.getHost()+"/"+helpURL;Context.extHelpURL='FALSE';}else{Context.helpURL=helpURL;Context.extHelpURL='TRUE';}}else{$('.helpText').css('display','none');}
